...
`include_session_layers=False` 在 Houdini 內特別有用：`/stage` 的 session layer 底下掛著 Viewport State、Solo Lights／Geometry、Purpose、Selectability 等一整組 viewport 用的 sublayer，關掉之後只會留下真正的內容 layer（實測一個簡單場景是 14 → 5），`pendingWrites` 不受影響。
