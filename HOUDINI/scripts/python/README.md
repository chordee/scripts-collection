...
| 檔案 | 覆蓋 | 環境 |
|---|---|---|
| `test_afanasy_submitter.py` | Base64 路徑、command 與 frame step、job/block 組裝、環境變數篩選注入（白名單與前綴比對、setEnv graceful fallback）、面板預設值、ROP 選取、存檔確認取消、錯誤訊息與視窗替換 | hython / plain Python；UI 與 af 使用 mock |
...
3. 儲存成功後，在 Houdini 內使用 `af` 建立一個 `af.Job` 和一個 service 為 `hbatch` 的 `af.Block`，設定 command、numeric frame range、capacity 與 priority。同時透過 `block.setEnv()` 將當前 session 中經過篩選的環境變數注入至該 block（包含精確名稱 `PATH`、`PYTHONPATH`、`SIDEFXLABS`、`HOUDINI_PATH`、`HOUDINI_CGRU_PATH`，以及符合前綴 `CGRU_*`、`RP_*`、`PUB_*`、`JOB_*`、`AXIOM_*`、`REZ_*` 的變數），既能確保 farm worker 上的 hython 完整重現專案、外掛與管線配置，又能避免無關系統變數或敏感資訊外流，最後呼叫 `job.send()`。
