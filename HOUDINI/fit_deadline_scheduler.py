    ...
    def auto_fit(self):
        ...
        if hou.applicationVersion()[0]==20 or hou.applicationVersion()[0]==19:
            ...
            if hou.applicationVersion()[0]==20 and hou.applicationVersion()[1]==5:
                node.parm('deadline_plugindirectory').set('Q:/Resource/H205_PDGDeadline')
            else:
...
def setCombBoxItems(combo, items):
    for ind, item in enumerate(items):
        combo.insertItem(ind, item)
...
if node.type().name() == 'deadlinescheduler':
    ...
    hfs = 'C:/Program Files/Side Effects Software/Houdini ' + \
    ...
    # env_parm = node.parm('deadline_envmulti')
    # alt_envs = ['HOUDINI_PATH', 'PATH', 'HOUDINI_DSO_ERROR', 'PXR_PLUGINPATH_NAME']
    # env_names = [x for x in alt_envs if x in os.environ.keys()]
    # setEnv(env_parm, env_names)

    deadlineBin = os.environ['DEADLINE_PATH']

    deadlineCommand = os.path.join(deadlineBin, "deadlinecommand.exe -Pools")
    proc = subprocess.Popen(deadlineCommand, stdin=subprocess.PIPE, stdout=subprocess.PIPE,
                            stderr=subprocess.PIPE, shell=False, creationflags=creationflags)
