...
class PortableReferences(base.OutputProcessor):
    ...
    def beginSave(self, config_node, config_overrides, lop_node, t, stage_variables):
        ...
        output_dir, _ = os.path.split(self.output_usd)
        # Normalize to forward slashes so the prefix comparison in
        # processReferencePath matches abs_asset_path (also forward-slashed
        # via hou.text.normpath) even when lopoutput uses backslashes.
        self.output_dir = hou.text.normpath(output_dir).replace('\\', '/') if output_dir else ''
    ...
    def processReferencePath(self, asset_path, referencing_layer_path, asset_is_layer):
        ...
        output_dir_norm = self.output_dir.rstrip('/') + '/'
